...
static HardwareInfo collectHardwareInfo() {
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            if (strncmp(line, "model name", 10) == 0 && h.cpuName[0] == 'U') {
                char* colon = strchr(line, ':');
                if (colon) {
                    colon++; while (*colon == ' ') colon++;
                    char* nl = strchr(colon, '\n'); if (nl) *nl = 0;
                    strncpy(h.cpuName, colon, sizeof(h.cpuName) - 1);
                }
            }
            if (strncmp(line, "cpu MHz", 7) == 0 && h.cpuMaxMHz == 0) {
                char* colon = strchr(line, ':');
                if (colon) h.cpuMaxMHz = (int)atof(colon + 1);
            }
            if (strncmp(line, "cpu cores", 9) == 0 && h.cpuCores == 0) {
                char* colon = strchr(line, ':');
                if (colon) h.cpuCores = atoi(colon + 1);
            }
        }
        fclose(f);
    }
...
int main() {
    ...
    for (const auto& d : DENSITIES) {
        ...
        for (int n : SIZES) {
            ...
            char label[64];
            snprintf(label, sizeof(label), "%-6s n=%-6d", d.id, n);
